...
async def test_transaction_history():
    ...
    try:
        ...
        print(f"  Current balance: {summary['current_balance']} credits (${summary['current_balance'] / CREDITS_PER_DOLLAR:.2f})")
        ...
        for i, txn in enumerate(all_transactions[:5], 1):  # Show first 5
            print(f"  {i}. {txn['transaction_type']:12} | "
        ...
        for i, txn in enumerate(deductions[:3], 1):  # Show first 3
            print(f"  {i}. Model: {txn['model_name'] or 'N/A':20} | "
                  f"Credits: {abs(txn['amount']):4} | "
                  f"USD: ${txn['usd_cost']:.4f}" if txn['usd_cost'] else f"USD: N/A")
        ...
        for i, txn in enumerate(additions[:3], 1):  # Show first 3
            print(f"  {i}. Amount: +{txn['amount']} credits | "

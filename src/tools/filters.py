    ...
    for sym in symbols:
        try:
            ...
            if balanced_growth or high_earnings_growth or high_revenue_growth:
                ...
                print(f"  ✓ PASSED")
                if balanced_growth:
                    print(f"    - Balanced growth (Rev > 8%, Earn > 10%)")
                if high_earnings_growth:
                    print(f"    - High earnings growth (> 20%)")
                if high_revenue_growth:
                    print(f"    - High revenue growth (> 15%)")
            else:
                print(f"  ✗ FAILED: Growth criteria not met")
...
def value_filter(symbols):
    ...
    for sym in symbols:
        try:
            ...
            print(f"  Price to Sales: {price_to_sales:.2f}" if price_to_sales else "  Price to Sales: N/A")
            
            # Multiple paths to qualify as value stock
            reasonable_pe_with_low_multiple = (
                trailing_pe is not None and trailing_pe < 20 and
                ((price_to_book is not None and price_to_book < 5) or 
                 (price_to_sales is not None and price_to_sales < 3))
            )
            ...
            if reasonable_pe_with_low_multiple or deep_book_value or attractive_forward_pe:
                ...
                if reasonable_pe_with_low_multiple:
                    print(f"    - Reasonable PE with low multiples")
                if deep_book_value:
                    print(f"    - Deep book value (P/B < 2.5)")
                if attractive_forward_pe:
                    print(f"    - Attractive forward PE (< 15)")
            else:
                print(f"  ✗ FAILED: Value criteria not met")

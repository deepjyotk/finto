...
def upsert_symbols_from_iterable(index, embeddings: HuggingFaceEmbeddings,
                                 items: Iterable[Tuple[str, str]], batch_size: int = 64):
    """
    ...
    for symbol, company in items:
        ...
        if len(ids) >= batch_size:
            ...
            to_upsert = [{"id": ids[i], "values": vectors[i], "metadata": metas[i]} for i in range(len(ids))]
...
def upsert_from_portfolio_excel(index, embeddings: HuggingFaceEmbeddings,
                                excel_path: str = "portfolio.xlsx", symbol_col: str = "Symbol", name_col: str = "Name"):

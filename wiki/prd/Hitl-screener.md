## Screener Agent HITL Flow Requirement

Currently, when a user asks the agent for screening, the UI should generate a form with parameters defined in _MEDIUM_DEFAULTS, pre-populated with those values. The user should be able to edit the form and click submit—introducing a human-in-the-loop to allow flexibility before the tool executes.

When screener_analysis_tool_node receives a sub-task from orchestration, the flow should be:

First, remove the execute_code_tool implementation from screener_analysis_tool_node, as we are moving toward a more deterministic get_screened_stocks tool. Therefore, the execute code logic is no longer needed.

```
orchetration_node --> screener_analysis_tool_node --> HITL_screener --> get_screened_stocks --> screener_analysis_tool_node --> orchestration
```

To implement HITL in LangGraph/LangChain, follow the docs: [https://docs.langchain.com/oss/python/langchain/human-in-the-loop](https://docs.langchain.com/oss/python/langchain/human-in-the-loop). Also, use AsyncPostgresSaver as the checkpointer, referring to: [https://github.com/langchain-ai/langgraph/tree/main/libs/checkpoint-postgres](https://github.com/langchain-ai/langgraph/tree/main/libs/checkpoint-postgres)
...
assert {"get_ticker_info", "get_balance_sheet", "get_income_statement", "get_earnings_estimate", "get_revenue_estimate"} <= _ALLOWED_WRAPPERS
...
def _screen_one(
    ...
    *,
    ...
    if rev_g is None:
        rev_est = get_revenue_estimate(symbol).get("revenue_estimate") or {}
        if isinstance(rev_est, dict):
            rev_g = _estimate_growth_pct(rev_est)
        snap["revenue_growth_pct"] = rev_g

    if eps_g_info is None:
        earn_est = get_earnings_estimate(symbol).get("earnings_estimate") or {}
        if isinstance(earn_est, dict):
    ...
    inc = get_income_statement(symbol, freq="yearly").get("income_statement") or {}
    latest_i = _latest_period_row(inc if isinstance(inc, dict) else {})
    ...
    bs = get_balance_sheet(symbol, freq="yearly").get("balance_sheet") or {}
    latest_b = _latest_period_row(bs if isinstance(bs, dict) else {})
...
def run_get_screened_stocks_sync(
    *,
    ...
    symbols: list[str] = list(runtime.context.get("screener_candidate_symbols") or [])
